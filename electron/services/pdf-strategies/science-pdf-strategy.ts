...
import { buildScienceEpdfPageUrl } from '../science-pdf.js';
...
function throwScienceDownloadFailure(
  ...
  throw appError('PDF_DOWNLOAD_FAILED', {
    ...
    attemptedUrls: request.scienceCandidateUrls,
    failures,
  });
}
...
function resolveStrictScienceDownloadTargets(request: PdfDownloadRequest) {
  const scienceValidationUrl =
    buildScienceEpdfPageUrl(request.pageUrl, request.doi) ||
    (request.explicitDownloadUrl
      ? buildScienceEpdfPageUrl(request.explicitDownloadUrl, request.doi)
      : null) ||
    request.pageUrl ||
    request.explicitDownloadUrl ||
    request.scienceCandidateUrls[0] ||
    '';
  const preferredScienceDownloadUrl =
    request.scienceCandidateUrls[0] || request.explicitDownloadUrl || '';
...
async function downloadSciencePdf(request: PdfDownloadRequest): Promise<PdfDownloadResult> {
  return await runSerializedSciencePdfDownload(request.pageUrl, async () => {
    logSciencePdf('start', {
      pageUrl: request.pageUrl,
      explicitDownloadUrl: request.explicitDownloadUrl,
      doi: request.doi,
      directCandidateUrls: request.scienceCandidateUrls,
      downloadDir: request.downloadDir,
      strategy: 'validated-window-primary',
    });

    const failures: PdfDownloadAttemptFailure[] = [];
    const strictTargets = resolveStrictScienceDownloadTargets(request);
    ...
    const directDownloadAttempt =
      request.scienceCandidateUrls.length > 0
        ? await tryDownloadPdfCandidates(request.scienceCandidateUrls, request.pageUrl)
...
export const sciencePdfStrategy: PdfDownloadStrategy = {
  ...
  matches(request) {
    return request.scienceCandidateUrls.length > 0;
  },
