...
import { natureResearchPdfStrategy } from './pdf-strategies/nature-research-pdf-strategy.js';
import { sciencePdfStrategy } from './pdf-strategies/science-pdf-strategy.js';
import type { PdfDownloadRequest, PdfDownloadStrategy } from './pdf-strategies/types.js';
import { buildScienceDirectPdfDownloadCandidates } from './science-pdf.js';
...
const pdfDownloadStrategies: readonly PdfDownloadStrategy[] = [
  sciencePdfStrategy,
  natureResearchPdfStrategy,
];

function resolveMatchingPdfDownloadStrategies(request: PdfDownloadRequest) {
  return pdfDownloadStrategies.filter((strategy) => strategy.matches(request));
}

async function previewDownloadPdfWithResolvedRequest(request: PdfDownloadRequest) {
  const directCandidateUrls = [
    ...new Set([
      ...request.natureCandidateUrls,
      request.explicitDownloadUrl,
  ...
  if (!downloaded) {
    let html =
      typeof request.previewPageHtml === 'string' && request.previewPageHtml.trim()
        ? request.previewPageHtml
        : '';
    if (!html) {
      try {
...
function createPdfDownloadRequest(
  ...
  return {
    ...
    downloadDir,
    previewPageHtml,
    scienceCandidateUrls: [
      ...new Set([
        ...buildScienceDirectPdfDownloadCandidates(pageUrl, doi),
        ...(explicitDownloadUrl
          ? buildScienceDirectPdfDownloadCandidates(explicitDownloadUrl, doi)
          : []),
      ]),
    ],
    natureCandidateUrls: [
      ...new Set([
        ...
        ...(explicitDownloadUrl ? buildNatureResearchPdfDownloadCandidates(explicitDownloadUrl) : []),
      ]),
    ],
  };
}
...
export async function previewDownloadPdf(
  ...
  const request = createPdfDownloadRequest(payload, defaultDownloadDir, previewPageHtml);
  ...
  logPdfStrategy('request_built', {
    ...
    scienceCandidateCount: request.scienceCandidateUrls.length,
    natureCandidateCount: request.natureCandidateUrls.length,
  });
  ...
  const exclusiveStrategy =
    matchingStrategies.find((strategy) => strategy.disposition === 'exclusive') ?? null;
  ...
  const preferredStrategies = matchingStrategies.filter(
    (strategy) => strategy.disposition === 'preferred',
  );
